...
if __name__ == '__main__':
    ...
    parser.add_argument('--corruption', metavar='X', type=float, default=0,
                       help='corrupt input pixels with probability X')
    ...
    global coder
    coder = getattr(networks, args.model)(n_hidden=args.hidden,
                                          fromfile=args.params)
    
    sess = tf.Session()
    with sess.as_default():
        train.train(sess, coder, dataset[:50000], dataset[50000:51000],
                    training_epochs=args.epochs, batch_size=args.batch,
                    learning_rate=args.learning_rate, corruption=args.corruption,
        ...
        if hasattr(coder, 'features'):
            ...
            if args.output is None:
                pyplot.imshow(tiles)
                pyplot.show()
            else: pyplot.imsave(args.output+'weights.png', tiles)
